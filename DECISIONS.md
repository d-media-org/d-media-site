...
Детайлните страници на услугите използват тематични свързани предложения, с максимум четири записа и без връзка към текущата услуга. `/services/content-social-media/` е комбиниран обзор, а не трета равностойна услуга, и насочва към страниците за създаване на съдържание и управление на социални медии. `/services/additional-charges-rights/` е информационна страница, а не основна услуга. Съществуващите маршрути и цени се запазват.

### Причина

Целта е да се намали дублирането и да се разграничат свързани услуги и информационни страници без миграция на маршрути.

### Последствия

Маршрутите и ценовите стойности остават непроменени.
...
„Работа“ е общият публичен концептуален слой за проекти и казуси. `/projects/` е главният индекс, а `/case-studies/` е вторичен индекс само за задълбочени проекти, които покриват достатъчен съдържателен праг. Казус няма отделен детайлен адрес: всички записи използват каноничния маршрут `/projects/<slug>/`, а статусът се задава в самите проектни данни.

### Причина

Кратките портфолио записи и задълбочените истории трябва да останат част от една система, без паралелни детайлни адреси или несъгласувани списъци.

### Последствия

Допълнителните секции се показват само за проекти с изричен статус „казус“ и достатъчна съдържателна основа. Съществуващите маршрути и канонични адреси се запазват.
...
Публичното представяне разграничава текущото маркетингово позициониране на d . media, етапите в историята на бранда и етапите в развитието на уеб платформата. Историите на бранда и платформата са редакционни разкази, а не дневници на вътрешни промени. Публичният разказ за бранда не назовава по-ранни идентичности или физическото лице зад него.

### Причина

Текущото предложение, развитието на бранда и техническата история на сайта са различни теми и трябва да се представят отделно, без лична идентификация или разкриване на вътрешния дневник.

### Последствия

Страницата „За бранда“ представя позиционирането и историята на бранда. Страниците `/site-history/` и `/site-history/daily/` представят етапи от развитието на платформата и не извеждат съдържанието на `SITE_HISTORY.md` или `SITE_HISTORY_DAILY.md`. Съществуващите адреси се запазват.
