...
export const publishedEnPosts: BlogPost[] = seeds.map((seed) => {
  ...
  return {
    ...
    ctaTitle: `${seed.category}: a practical next step`,
    ctaText: `Share the project, required outcome, current situation, and timing. We will connect the subject to ${seed.category.toLowerCase()} and suggest a concrete next step.`,
